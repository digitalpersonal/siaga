...
const ReportsAnalytics: React.FC = () => {
    ...
    const metrics = useMemo(() => {
        ...
        const noShow = appointments.filter(a => a.transportStatus === 'absent').length; // Assuming explicit absent mark
        // Or calculate pending as potential no-shows if date is past
        
        // Calculate service popularity
        const serviceCounts: Record<string, number> = {};
        appointments.forEach(a => {
            const name = a.service_name.split('(')[0].trim(); // Normalize "Cardiologia (Dr X)" to "Cardiologia"
...
const ProfessionalManagement: React.FC = () => {
    ...
    const handleOpenQuickBook = async (user: User) => {
        ...
        // Ensure user is treated as ProfessionalUser (assuming fetched data contains necessary fields)
        const professionalUser = user as ProfessionalUser;
        
        // Fetch appointments to calculate slots
    ...
    const handleQuickBookSuccess = () => {
        // alert('Agendamento realizado com sucesso!'); // Feedback already inside modal
